...
//9-3에서 수정됨
    ...
    public String index(Model model)
    {
        ...
        if(user!=null) //user가 있을경우에 대한 코드
...
//    @GetMapping("/posts/relaySave")
//    //url 맨뒤에 /posts/save가 붙는다면. (ex.http://www.naver.com/posts/save)
//    public String postsRelaySave()
//    {
//        return  "posts-relaySave";
//    }



    @GetMapping("/posts/relay/{id}") //p11-1 23:10
    ...
    @GetMapping("/posts/relay/{id}/relayUpdate") //p11-1 23:10
    public String postsRelayUpdate(@PathVariable Long id, Model model) //sub-templatate에서 저장되는건 무조건 model 타입으로 저장되야
    {
        PostsResponseDto dto = postsService.findById(id); //id를 찾아가지고 response로 보내라
        model.addAttribute("posts", dto); //dto형태로 데이터를 저장하겠다
        return  "posts-relayUpdate";
    }
